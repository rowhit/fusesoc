...
#Get available tool options from backends? Require either a spec, or having the backends available. The latter breaks frontend/backend separation
import importlib
import logging
import os
...
class Core:
    def __init__(self, core_file, cache_root, build_root):
        ...
        for fs in self.filesets.values():
            for f in fs.files:
                f.logical_name = fs.logical_name
    ...
    def get_tool_options(self, flags):
        _flags = flags.copy()
        #_flags['is_toplevel'] = True #FIXME: Is this correct? No! This gets the wrong target for non-toplevel cores. What was the idea behind this?? Can't remember
    ...
    def get_toplevel(self, flags):
        ...
        target = self._get_target(_flags)
        if target.toplevel:
            self._debug("Matched toplevel {}".format(target.toplevel))
